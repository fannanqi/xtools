...
{
    // Initialize the color list with different colors
    m_colorList << Qt::red << Qt::green << Qt::blue << Qt::yellow << Qt::cyan << Qt::magenta << Qt::gray;
...
CanttTimeBarItem *CanttChartScene::addPlanTimeBar(const QString &platform, const QDateTime &start, const QDateTime &end)
{
    ...
    // Determine the color for the current bar based on platform index
    int platformIndex = m_platforms.indexOf(platform);
    QColor color = m_colorList.at(platformIndex % m_colorList.size()); // Use modulo to cycle through colors



    //绘制时间条图形项
    CanttTimeBarItem *item = new CanttTimeBarItem(start, end, CanttTimeBarItem::PlanTime, color, m_perPixelHMsecs);
...
CanttTimeBarItem *CanttChartScene::addRealTimeBar(const QString &platform, const QDateTime &start, const QDateTime &end)
{
    ...
    //绘制时间条图形项
    CanttTimeBarItem *item = new CanttTimeBarItem(start, end, CanttTimeBarItem::RealTime, Qt::green, m_perPixelHMsecs);
