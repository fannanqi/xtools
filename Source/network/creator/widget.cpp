    ...
    isEditMode(false)
{
    ...
    ensureDirectoryExists("D:/c++/StoreData");
    // 加载设备管理数据
    loadDevDataFromFile("D:/c++/StoreData/device_data.xml");
    // 加载报文管理数据
    loadPacDataFromFile("D:/c++/StoreData/pac_data.xml");
    // 加载参数管理数据
    loadParDataFromFile("D:/c++/StoreData/par_data.xml");
    // 初始化表格
    refreshTable();
    // 设置虚化的汉字提示
...
Widget::~Widget()
{
    ensureDirectoryExists("D:/c++/StoreData");
    // 保存设备管理数据
    saveDevDataToFile("D:/c++/StoreData/device_data.xml");
    // 保存报文管理数据
    savePacDataToFile("D:/c++/StoreData/pac_data.xml");
    // 保存参数管理数据
    saveParDataToFile("D:/c++/StoreData/par_data.xml");
...
void Widget::onDeleteButtonClicked(int row)
{
    ...
    if (ui->lb_dev->styleSheet() == "color:white") {//"color:red") {
        activeLabel = ui->lb_dev;
        activeData = &dev_Data;
        filename = "device_data.xml";
    } else if (ui->lb_pac->styleSheet() == "color:white") {//"color:red") {
        activeLabel = ui->lb_pac;
        activeData = &pac_Data;
        filename = "pac_data.xml";
    } else if (ui->lb_par->styleSheet() == "color:white") {//"color:red") {
